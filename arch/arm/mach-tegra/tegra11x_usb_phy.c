...
static int utmi_phy_power_on(struct tegra_usb_phy *phy)
{
	...
	utmi_xcvr_setup = phy->utmi_xcvr_setup;

	if(phy->inst == 0 || phy->inst == 2) {
		dock_in = !(gpio_get_value(TEGRA_GPIO_PO0));
		if(phy->inst == 0) {
			if(dock_in == 0)
				utmi_xcvr_setup = utmi_xcvr_setup - 3;
			else
				utmi_xcvr_setup = utmi_xcvr_setup - 7;
		} else if(phy->inst == 2) {
			if(dock_in == 0)
				utmi_xcvr_setup = utmi_xcvr_setup - 6;
		}
		if(utmi_xcvr_setup < 0)
			utmi_xcvr_setup = 0;

		pr_info("phy->inst = %d , phy->utmi_xcvr_setup = %d\n", phy->inst, utmi_xcvr_setup);
	}
...
//	val |= UTMIP_XCVR_SETUP(phy->utmi_xcvr_setup);
	val |= UTMIP_XCVR_SETUP(utmi_xcvr_setup);
