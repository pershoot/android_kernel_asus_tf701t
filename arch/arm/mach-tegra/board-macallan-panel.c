...
#define hdmi_vbus_en_oc TEGRA_GPIO_PP2
...
static void macallan_panel_select(void)
{
	...
	if (machine_is_mozart()) {
		...
		printk("Display: macallan_panel_select is haydn \n");
		panel = &hdmi_haydn_1080p;
	}
...
int __init macallan_panel_init(void)
{
	...
	err = gpio_request(hdmi_vbus_en_oc, "HDMI_VBUS_EN_OC#");
	if (err < 0) {
		pr_err("HDMI_VBUS_EN_OC# request failed\n");
	}
	gpio_direction_output(hdmi_vbus_en_oc, 1);
