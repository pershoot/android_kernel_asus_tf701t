...
/* Used for dbgfs.
 * Write a register offset here before you cat /d/ov5693/get_reg */
static u16 g_read_offset = 0xFFFF;
...
static const struct ov5693_reg ov5693_2592x1944_i2c[] = {
	...
	{0x3814, 0x11},
	{0x3815, 0x11},
	{0x3820, 0x42}, /* Flip: [6]=b1; [1]=b1 */
	{0x3821, 0x18}, /* Mirror: [2:1]=b00 */
	{0x3823, 0x00},
	{0x3824, 0x00},
	{0x3825, 0x00},
	{0x3826, 0x00},
...
static const struct ov5693_reg ov5693_1296x972_i2c[] = {
	...
	{0x3814, 0x31},
	{0x3815, 0x31},
	{0x3820, 0x43}, /* Flip: [6]=b1; [1]=b1 */
	{0x3821, 0x19}, /* Mirror: [2:1]=b00 */
	{0x3823, 0x00},
	{0x3824, 0x00},
	{0x3825, 0x00},
	{0x3826, 0x00},
...
static const struct ov5693_reg *mode_table[] = {
	...
	[OV5693_MODE_640x480_60FPS]	= ov5693_640x480_60fps_i2c,
		...
		struct ov5693_mode *mode)
{
	...
	if (mode->hdr_en == true) {
		...
		else if (mode->res_x == 640 && mode->res_y == 480)
			mode_index = OV5693_MODE_640x480_60FPS;
	}

	if (!info->mode_valid || (info->mode_index != mode_index)) {
		pr_info("%s(%d): mode_index=%d; (%dx%d)\n",
			__func__, __LINE__,
			mode_index, mode->res_x, mode->res_y);
...
static int ov5693_get_fuse_id(struct ov5693_info *info)
{
	...
	/* Todo:
	 * ProjectID check must be necessary. Bypass fuse id for TF501 camera
	 * calibration only. */
	info->fuseid.id[0] = 0;
	info->fuseid.id[1] = 0;
#else
...
static ssize_t dbg_ov5693_camera_status_open(
...
static ssize_t dbg_ov5693_camera_status_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
	...
	err = ov5693_i2c_rd8(info, 0x300A, &chip_id_HB);
	err |= ov5693_i2c_rd8(info, 0x300B, &chip_id_LB);

	if (err) {
		len = snprintf(bp, dlen, "0\n");
		tot += len; bp += len; dlen -= len;
		pr_info("%s(%d): err=%d\n", __func__, __LINE__, err);
	} else if ((chip_id_HB == 0x56) && (chip_id_LB == 0x90)) {
		len = snprintf(bp, dlen, "1\n");
		tot += len; bp += len; dlen -= len;
	} else {
		len = snprintf(bp, dlen, "0\n");
		tot += len; bp += len; dlen -= len;
		pr_info("%s(%d): wrong chip_id: HB=0x%02X; LB=0x%02X\n",
			__func__, __LINE__, chip_id_HB, chip_id_LB);
	}
...
static const struct file_operations dbg_ov5693_camera_status_fops = {
	.open		= dbg_ov5693_camera_status_open,
	.read		= dbg_ov5693_camera_status_read,
};

static ssize_t dbg_get_ov5693_reg_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static ssize_t dbg_get_ov5693_reg_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
{
	...
	err = ov5693_i2c_rd8(info, g_read_offset, &reg_val);

	if (err) {
		len = snprintf(bp, dlen, "err= %d\n", err);
		tot += len; bp += len; dlen -= len;
	} else {
		len = snprintf(bp, dlen,
			"[0x%04x]= 0x%x\n", g_read_offset, reg_val);
		tot += len; bp += len; dlen -= len;
	}
	if (copy_to_user(buf, debug_buf, tot))
		return -EFAULT;
...
static const struct file_operations dbg_get_ov5693_reg_fops = {
	.open		= dbg_get_ov5693_reg_open,
	.read		= dbg_get_ov5693_reg_read,
};


static ssize_t dbg_set_ov5693_reg_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static ssize_t dbg_set_ov5693_reg_write(
	struct file *file,
	...
	loff_t *ppos)
{
	...
	*b_ptr++ = reg_val;
	...
	regmap_bulk_write(info->regmap, i2c_reg, i2c_buf, buf_count);

	return count;
}


static const struct file_operations dbg_set_ov5693_reg_fops = {
	.open		= dbg_set_ov5693_reg_open,
	.write		= dbg_set_ov5693_reg_write,
};

static ssize_t dbg_dump_ov5693_otp_open(struct inode *inode, struct file *file)
{
	file->private_data = inode->i_private;
	return 0;
}

static ssize_t dbg_dump_ov5693_otp_read(
	struct file *file,
	char __user *buf,
	size_t count,
	loff_t *ppos)
...
static const struct file_operations dbg_dump_ov5693_otp_fops = {
	.open		= dbg_dump_ov5693_otp_open,
	.read		= dbg_dump_ov5693_otp_read,
};
...
void ov5693_dbgfs_init(void)
{
	struct dentry *debugfs_dir;

	debugfs_dir = debugfs_create_dir("ov5693", NULL);
	(void) debugfs_create_file("i2c_write", S_IRUGO | S_IWUSR,
		debugfs_dir, NULL, &dbg_set_ov5693_reg_fops);
	(void) debugfs_create_file("i2c_read", S_IRUGO,
		debugfs_dir, NULL, &dbg_get_ov5693_reg_fops);

	debugfs_create_x16("read_offset", 0644, debugfs_dir, &g_read_offset);

	(void) debugfs_create_file("camera_status", S_IRUGO,
		debugfs_dir, NULL, &dbg_ov5693_camera_status_fops);
	(void) debugfs_create_file("CameraOTP", S_IRUGO,
		debugfs_dir, NULL, &dbg_dump_ov5693_otp_fops);
}

void ov5693_otp_dump(void)
{
	int err = 0;
	int i = 0, j = 0;
	struct ov5693_power_rail *pw = &info->regulators;
	...
	err = ov5693_i2c_wr_table(info, mode_table[0]);

	msleep(15);

	for (j = 3; j > 0; j--) {
		/* set bank */
		err = regmap_write(info->regmap, 0x3D84, 0xC0 + j);
		err = regmap_write(info->regmap, 0x3D81, 0x01);
		msleep(15);
		...
		for (i = 0; i < 10; i++) {
			u8 otp_val = 0;
			msleep(3);
			err = ov5693_i2c_rd8(info, 0x3D00 + i, &otp_val);
			otp[i] = otp_val;
		}
		if (otp[9] != 0x0) {
			pr_info("bank[%d] is ok\n", j);
			break;
		} else if (j == 1)
			pr_info("bank[1] is invalid. No AF Data?\n");
		else
			pr_info("bank[%d] is invalid. Find the next...\n", j);
	}
	err = regmap_write(info->regmap, 0x0100, 0x00);
...
int __init ov5693_late_init(void)
{
	if(!machine_is_haydn()) {
		pr_info("%s++\n", __func__);
...
late_initcall(ov5693_late_init);
