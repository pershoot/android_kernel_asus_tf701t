 ...
 * the closest refresh rate. Further, if more than 1 videomode with
 * closest refresh rate is found, will return the videomode with
 * the closest pixel clock.
 */
					        ...
					        struct list_head *head)
{
	...
	u32 diff = -1, diff_refresh = -1, diff_pixclock = -1;
	...
	list_for_each(pos, head) {
		...
		if (diff > d) {
			...
			diff_pixclock = abs(cmode->pixclock - mode->pixclock);
			best = cmode;
		} else if (diff == d) {
			...
			if (diff_refresh > d) {
				...
				d = abs(cmode->pixclock - mode->pixclock);
				if(diff_pixclock > d){
					diff_pixclock = d;
					best = cmode;
