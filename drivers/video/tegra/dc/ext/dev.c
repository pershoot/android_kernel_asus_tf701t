			   ...
			   unsigned long arg)
{
	...
	case TEGRA_DC_EXT_SET_CMU:
	{
        ...
        case TEGRA_DC_EXT_SET_CMU_CSC:
        {
#if !defined(CONFIG_ARCH_TEGRA_2x_SOC) && !defined(CONFIG_ARCH_TEGRA_3x_SOC)
                int ret = 0;
                struct tegra_dc_ext_cmu_csc *args;
                struct tegra_dc *dc = user->ext->dc;
                ...
                dc->pdata->cmu->csc.krr = args->csc[0];
                dc->pdata->cmu->csc.kgr = args->csc[1];
                dc->pdata->cmu->csc.kbr= args->csc[2];
                dc->pdata->cmu->csc.krg= args->csc[3];
                dc->pdata->cmu->csc.kgg= args->csc[4];
                dc->pdata->cmu->csc.kbg= args->csc[5];
                dc->pdata->cmu->csc.krb= args->csc[6];
                dc->pdata->cmu->csc.kgb= args->csc[7];
                dc->pdata->cmu->csc.kbb = args->csc[8];

                tegra_dc_writel(dc, args->csc[0], DC_COM_CMU_CSC_KRR);
                tegra_dc_writel(dc, args->csc[1], DC_COM_CMU_CSC_KGR);
                tegra_dc_writel(dc, args->csc[2], DC_COM_CMU_CSC_KBR);
                tegra_dc_writel(dc, args->csc[3], DC_COM_CMU_CSC_KRG);
                tegra_dc_writel(dc, args->csc[4], DC_COM_CMU_CSC_KGG);
                tegra_dc_writel(dc, args->csc[5], DC_COM_CMU_CSC_KBG);
                tegra_dc_writel(dc, args->csc[6], DC_COM_CMU_CSC_KRB);
                tegra_dc_writel(dc, args->csc[7], DC_COM_CMU_CSC_KGB);
                tegra_dc_writel(dc, args->csc[8], DC_COM_CMU_CSC_KBB);
