...
void tegra_dc_enable(struct tegra_dc *dc)
{
	...
	printk("Display check tegra_dc_enable X \n");
	mutex_unlock(&dc->lock);
	trace_display_mode(dc, &dc->mode);
	display_on_flag = 1;
}
...
void tegra_dc_disable(struct tegra_dc *dc)
{
	...
	printk("Display check tegra_dc_disable E \n");
	...
	printk("Display check tegra_dc_disable X \n");
	mutex_unlock(&dc->lock);
...
static int tegra_dc_probe(struct platform_device *ndev)
{
	...
	if (!machine_is_haydn() && dc->out_ops && dc->out_ops->detect)
...
static ssize_t camera_show_power(struct class *class,
		struct class_attribute *attr, char *buf)
{
	int rc = -ENXIO;
	int value = 0;
	value = gpio_get_value(EN_1V8_CAM);
	rc = sprintf(buf, "%d\n",value);
	return rc;
}

static ssize_t camera_store_power(struct class *class,
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc)
		return rc;
	gpio_set_value(EN_1V8_CAM, val);
	rc = count;
	pr_info("USB Camera Power =%ld \n",val);

	return rc;
}

static const struct class_attribute usb_camera_attributes = {
	.attr = {
		.name = "power",
		.mode = 0644,
	},
	.show = camera_show_power,
	.store = camera_store_power,
	.namespace = NULL,
};
...
static int __init tegra_dc_module_init(void)
{
	...
	if (machine_is_haydn()) {
		pr_info("Create sysfs of usb camera \n");
		usb_camera_class = class_create(THIS_MODULE, "usb_camera");
		if (IS_ERR(usb_camera_class)) {
			pr_err("Unable to create usb_camera class; errno = %ld\n",
