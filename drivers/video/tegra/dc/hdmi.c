...
static struct wake_lock hdmi_hotplug_wakelock;
					...
					struct fb_videomode *mode)
{
	...
	// since haydn's main display is HDMI, the resolution should fix to 1080p
	if(machine_is_haydn()
						...
						struct fb_monspecs *specs)
{
	...
	if (machine_is_haydn()) {
		// do not tell userspace there is an HDMI plugged in
...
static irqreturn_t tegra_dc_hdmi_irq(int irq, void *ptr)
{
	...
	wake_lock_timeout(&hdmi_hotplug_wakelock, 3*HZ);
...
static int tegra_dc_hdmi_init(struct tegra_dc *dc)
{
	...
	if(!machine_is_haydn()) {
		...
		dev_info(&dc->ndev->dev, "haydn not need hdcp\n");
		hdmi->nvhdcp = NULL;
	}
#else
	hdmi->nvhdcp = NULL;
	...
	if(!machine_is_haydn()) {
		hotplug_irq = gpio_to_irq(dc->out->hotplug_gpio);
		if (request_irq(hotplug_irq, tegra_dc_hdmi_irq,
...
static void tegra_dc_hdmi_destroy(struct tegra_dc *dc)
{
	...
	wake_lock_destroy(&hdmi_hotplug_wakelock);
