...
static irqreturn_t carddetect_irq(int irq, void *data)
{
	...
	pr_info("%s: carddetect_irq gpio_%d:%d\n", mmc_hostname(sdhost->mmc),
		plat->cd_gpio, gpio_get_value(plat->cd_gpio));
...
static int sdhci_tegra_execute_tuning(struct sdhci_host *sdhci, u32 opcode)
{
	...
	/*
	 * FIXME: We restrict max_clk_limit to 82MHz due to hw limitation.
	 * Thus, we don't need to tune tap window value even SDR50/104 mode.
	 */
	if (!strcmp(mmc_hostname(sdhci->mmc), "mmc1") ||
		!strcmp(mmc_hostname(sdhci->mmc), "mmc2"))
		return 0;
...
static int __devinit sdhci_tegra_probe(struct platform_device *pdev)
{
	...
	pltfm_host->priv = tegra_host;
	tegra_host->clk_enabled = true;
	if (machine_is_haydn() && !strcmp(mmc_hostname(host->mmc), "mmc0")) {
		revision = asustek_get_hw_rev();
		switch (revision) {
		case HW_REV_A:	// v1.0
		case HW_REV_E:	// v1.1
			pr_info("%s: temporary downgrade frequency for"
				" several hw version\n", mmc_hostname(host->mmc));
			tegra_host->max_clk_limit = 102000000;
			break;
		case HW_REV_C:	// v1.2
		case HW_REV_G:	// v1.3
		case HW_REV_B:	// v1.4
	...
	host->mmc->caps |= MMC_CAP_WAIT_WHILE_BUSY | MMC_CAP_ERASE;
