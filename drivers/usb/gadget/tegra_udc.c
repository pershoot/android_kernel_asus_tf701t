...
enum tegra_connect_type usb_cable_status = CONNECT_TYPE_NONE;
static ATOMIC_NOTIFIER_HEAD(cable_status_notifier_list);
extern int vbus_val_flag;
extern int pmu_vbus_status;
...
static int tegra_vbus_session(struct usb_gadget *gadget, int is_active)
{
	...
	if ((udc->connect_type != CONNECT_TYPE_NONE) && (pmu_vbus_status == 0)) {
		printk(KERN_INFO "USB cable/charger disconnected\n");
	}
	else if ((usb_cable_status != udc->connect_type) && (vbus_val_flag == 1)) {
		vbus_val_flag = 0;
		cable_status_notifier_call_chain(udc->connect_type, udc);
	}
	usb_cable_status = udc->connect_type;
	mutex_unlock(&udc->sync_lock);

	return 0;
}

unsigned int suspend_to_suspend_cable_report(void)
{
	vbus_val_flag = 0;
	cable_status_notifier_call_chain(CONNECT_TYPE_NONE, the_udc);
	usb_cable_status = CONNECT_TYPE_NONE;
	return 0;
}
EXPORT_SYMBOL(suspend_to_suspend_cable_report);
