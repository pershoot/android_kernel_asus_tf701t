...
static void gpio_dock_in_init()
{
	int ret = 0;
	ret = gpio_request(TEGRA_GPIO_PO0, "DOCK_IN");
	if (ret < 0)
		printk(KERN_ERR "DOCK_IN GPIO%d request fault!%d\n", TEGRA_GPIO_PO0, ret);
	ret= gpio_direction_input(TEGRA_GPIO_PO0);
	if (ret)
		printk(KERN_ERR "gpio_direction_input failed for input TEGRA_GPIO_PO0=%d\n", TEGRA_GPIO_PO0);
}
...
static int tegra_ehci_probe(struct platform_device *pdev)
{
	...
	/* Enable USB-HUB and PAD-USB-Port at P1802 booting.
	*  USB-HUB : GPIO_PBB5.
	...
	*/
	if (machine_is_haydn()) {
		...
		case HW_REV_E: break;
		default:
			ret = gpio_request(TEGRA_GPIO_PBB5, "GPIO_PBB5");
			if (ret < 0)
				printk(KERN_ERR "%s: Failed to gpio_request for GPIO_PBB5.\n", __func__);
			ret = gpio_direction_output(TEGRA_GPIO_PBB5, 1);
			if (ret)
				printk(KERN_ERR "%s: Failed to gpio_direction_output for GPIO_PBB5.\n", __func__);
			break;
		}

		ret = gpio_request(TEGRA_GPIO_PI7, "GMI_WAIT");
		if (ret < 0)
			printk(KERN_ERR "%s: Failed to gpio_request for GMI_WAIT.\n", __func__);
		ret = gpio_direction_output(TEGRA_GPIO_PI7, 1);
		if (ret)
			printk(KERN_ERR "%s: Failed to gpio_direction_output for GMI_WAIT.\n", __func__);
	}
...
static int tegra_ehci_resume(struct platform_device *pdev)
{
	...
	if (machine_is_haydn())
		gpio_set_value(TEGRA_GPIO_PI7, 1);
...
static int tegra_ehci_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (machine_is_haydn())
		gpio_set_value(TEGRA_GPIO_PI7, 0);
