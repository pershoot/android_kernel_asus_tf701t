...
static int idx=-1;
static int check_sisId_isReg = 1;
...
static int sis_ts_suspend(struct device *dev){
	pr_info("%s:++++\n", __func__);
	if(hid_dev_backup && gpio_get_value(TEGRA_GPIO_PO0)){
		usb_kill_urb( backup_urb );
		usb_free_urb( backup_urb );
		backup_urb = NULL;
		hid_hw_stop(hid_dev_backup);
		kfree(hid_get_drvdata(hid_dev_backup));
		hid_set_drvdata(hid_dev_backup, NULL);
		hid_dev_backup = NULL;
	}

	gpio_direction_output(TEGRA_GPIO_PH0, 0);
	pr_info("%s:----\n", __func__);
	return 0;
}
...
static int sis_tch_input_enable(struct input_dev *dev)
{
	int error=0;

	struct sis_data *nd = input_get_drvdata(dev);

	error = sis_ts_resume(nd->dev);
	if (error)
		dev_err(nd->dev, "%s: failed\n", __func__);

	return error;
}

static int sis_tch_input_disable(struct input_dev *dev)
{
	int error=0;

	struct sis_data *nd = input_get_drvdata(dev);

	error = sis_ts_suspend(nd->dev);
	if (error)
		dev_err(nd->dev, "%s: failed\n", __func__);

	return error;
}
//joseph_shieh: switch touch power on_off
//Because of earlysuspend is useless in new kernel driver version,
//so we need create this visual input device for power on/off
struct sis_data *sis_tch_input_init(struct device *dev){
		...
		input_dev->name = "Vendor_0457_Product_0817";
		input_dev->enable = sis_tch_input_enable;
		input_dev->disable = sis_tch_input_disable;
...
static int sis_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	...
	if(check_sisId_isReg){
		nd = sis_tch_input_init(&hdev->dev);
		if (IS_ERR(nd)) {
			dev_err(&hdev->dev, "SIS TS: Input Device Initialization Fail!\n");
			ret = PTR_ERR(nd);
		}
		check_sisId_isReg = 0;
	}
