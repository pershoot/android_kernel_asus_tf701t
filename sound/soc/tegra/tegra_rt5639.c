...
#include <mach/tegra_rt5639_pdata.h>
					...
					struct snd_pcm_hw_params *params)
{
	...
	if (err < 0) {
		...
		err = snd_soc_dai_set_pll(codec_dai, RT5639_SCLK_S_PLL1,
				RT5639_PLL1_S_MCLK, rate, 512 * srate);
		if (err < 0) {
			      ...
			      unsigned long action, void *dev)
{
	...
	if (jack == &tegra_rt5639_hp_jack) {
		if (action) {
			...
			else if (!strncmp(machine->pdata->codec_name, "rt5639",
									    6))
				status_jack = rt5639_headset_detect(codec, 1);
			...
			if (status_jack == RT5639_HEADPHO_DET ||
			    status_jack == RT5639_HEADPHO_DET)
					machine->jack_status |=
							SND_JACK_HEADPHONE;
			else if (status_jack == RT5639_HEADSET_DET ||
				 status_jack == RT5639_HEADSET_DET) {
					machine->jack_status |=
							SND_JACK_HEADPHONE;
					machine->jack_status |=
							SND_JACK_MICROPHONE;
					...
					struct snd_kcontrol *k, int event)
{
	...
	if(machine_is_roth()) {
		if (SND_SOC_DAPM_EVENT_ON(event)) {
			if(i2s_tfa) {
				if (codec_rt)
					snd_soc_update_bits(codec_rt, RT5639_PWR_DIG1, 0x0001, 0x0000);
				tegra_asoc_enable_clocks();
				Tfa9887_Powerdown(0);
				tegra_asoc_disable_clocks();
			}
...
static const struct snd_soc_dapm_widget tegra_rt5639_dapm_widgets[] = {
...
static const struct snd_soc_dapm_route tegra_rt5639_audio_map[] = {
...
static const struct snd_kcontrol_new tegra_rt5639_controls[] = {
...
static struct snd_soc_dai_link tegra_rt5639_dai[] = {
	[DAI_LINK_HIFI] = {
		.name = "RT5639",
		.stream_name = "RT5639 PCM",
		.codec_name = "rt5639.4-001c",
...
static struct snd_soc_card snd_soc_tegra_rt5639 = {
	...
	.controls = tegra_rt5639_controls,
	.num_controls = ARRAY_SIZE(tegra_rt5639_controls),
	.dapm_widgets = tegra_rt5639_dapm_widgets,
	.num_dapm_widgets = ARRAY_SIZE(tegra_rt5639_dapm_widgets),
	.dapm_routes = tegra_rt5639_audio_map,
	.num_dapm_routes = ARRAY_SIZE(tegra_rt5639_audio_map),
	.fully_routed = true,
};
...
static __devinit int tegra_rt5639_driver_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &snd_soc_tegra_rt5639;
...
static int __devexit tegra_rt5639_driver_remove(struct platform_device *pdev)
{
...
static struct platform_driver tegra_rt5639_driver = {
	...
	.remove = __devexit_p(tegra_rt5639_driver_remove),
};
