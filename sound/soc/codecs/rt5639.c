...
static bool isRecording_DMIC = false;
static bool isRecording_AMIC = false;
static unsigned int project_info = mozart;
struct snd_soc_codec *rt5639_audio_codec = NULL;
EXPORT_SYMBOL(rt5639_audio_codec);
bool  is_rt5639_suspended = false;
EXPORT_SYMBOL(is_rt5639_suspended);
struct rt5639_init_reg {
	u8 reg;
	u16 val;
};
#ifdef CONFIG_PM
static void rt5639_resume_work(struct work_struct *work);

static struct workqueue_struct *g_rt5639_resume_work_queue = NULL;
static DECLARE_WORK(g_rt5639_resume_work, rt5639_resume_work);
#endif
#ifdef USE_HEADSET_DEPOP
static bool headset_depop = false;
...
static ssize_t read_audio_codec_status_rt5639(struct device *dev,
				    struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", audio_codec_status);
}
DEVICE_ATTR(audio_codec_status_rt5639, S_IRUGO, read_audio_codec_status_rt5639, NULL);
...
static int rt5639_reg_init(struct snd_soc_codec *codec)
{
	...
	if ((project_info == mozart) || (project_info == haydn))
		snd_soc_update_bits(codec, RT5639_SPO_CLSD_RATIO,
			RT5639_SPO_CLSD_RATIO_MASK, 0x0004);
...
/* RECORDING STATES */
static const char *rt5639_recording_mode[] = {"Disable_DMIC","Disable_AMIC","DMIC", "AMIC"};

static const SOC_ENUM_SINGLE_DECL(rt5639_recording_enum, 0, 0, rt5639_recording_mode);
...
static int rt5639_get_gain(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int rt5639_set_gain(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int ret = 0;
	...
	case RT5639_RECORDING_DMIC_DIS:
		printk("%s(): End DMIC Recording\n", __func__);
		isRecording_DMIC = false;
		if (project_info == mozart) {
			snd_soc_write(codec, RT5639_DRC_AGC_1,0x2202);
	...
	case RT5639_RECORDING_DMIC:
		/* set dmic gain */
		isRecording_DMIC = true;
		printk("%s(): set DMIC parameter isRecording_DMIC = %d\n", __func__,isRecording_DMIC);
		if (project_info == mozart) {
			if (!spk_out_flag) {
				snd_soc_write(codec, RT5639_DRC_AGC_1,0xE0A0);
				snd_soc_write(codec, RT5639_DRC_AGC_2,0x0009);
...
static const struct snd_kcontrol_new rt5639_snd_controls[] = {
	...
	SOC_ENUM("DAC IF2 Data Switch", rt5639_if2_dac_enum),
		/* Set recording gain */
	SOC_ENUM_EXT("Recording Gain", rt5639_recording_enum,
		rt5639_get_gain, rt5639_set_gain),
#ifdef RT5639_REG_RW
	{
...
#ifdef USE_EQ
		rt5639_update_eqmode(codec,MOZART);
#endif
...
#ifdef USE_HEADSET_DEPOP
static void rt5639_headset_depop_work(struct work_struct *work) {
	pr_debug("%s: delay to unmute headset for depop\n", __func__);
	msleep(500);
	if (headset_depop) {
		snd_soc_update_bits(rt5639_audio_codec, RT5639_HP_VOL,
				RT5639_L_MUTE | RT5639_R_MUTE, 0);
		msleep(65);
		pr_debug("%s: unmute headset\n", __func__);
		headset_depop = false;
	}
}
#endif
...
printk("rt5639_probe\n");
	...
	rt5639->codec = codec;
	rt5639_audio_codec = codec;
#ifdef CONFIG_PM
	g_rt5639_resume_work_queue = create_workqueue("rt5639_resume");
#endif
#ifdef USE_HEADSET_DEPOP
	...
	ret = device_create_file(codec->dev, &dev_attr_audio_codec_status_rt5639);
	if (ret != 0) {
		dev_err(codec->dev,"Failed to create audio_codec_status sysfs files: %d\n", ret);
		return ret;
	}
	if(snd_soc_read(codec, RT5639_VENDOR_ID1) == 0x10ec)
		audio_codec_status = 1;
	else
		printk("%s: incorrect audio codec rt5639 vendor ID\n", __func__);
	return 0;
}
...
{
	int ret;
	ret = gpio_direction_output(TEGRA_GPIO_LDO1_EN, 1);
	if (ret){
		printk("Fail gpio_direction AUDIO_LDO1 %d\n",
			TEGRA_GPIO_LDO1_EN);
	}
	msleep(400);
	rt5639_set_bias_level(rt5639_audio_codec, SND_SOC_BIAS_STANDBY);
	is_rt5639_suspended = false;
	return;
}
...
static int rt5639_suspend(struct snd_soc_codec *codec, pm_message_t state)
{
	...
	dev_dbg(codec->dev, "rt5639_suspend+\n");
	rt5639_set_bias_level(codec, SND_SOC_BIAS_OFF);
	ret = gpio_direction_output(TEGRA_GPIO_LDO1_EN, 0);
	if (ret){
		printk("Fail gpio_direction AUDIO_LDO1 %d\n",
			TEGRA_GPIO_LDO1_EN);
	}
	is_rt5639_suspended = true;
	dev_dbg(codec->dev, "rt5639_suspend-\n");
	return 0;
}
...
{
	dev_dbg(codec->dev, "rt5639_resume+\n");
	rt5639_audio_codec = codec;
	queue_work(g_rt5639_resume_work_queue, &g_rt5639_resume_work);
	dev_dbg(codec->dev, "rt5639_resume-\n");
	return 0;
}
#else
